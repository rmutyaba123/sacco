...
entryFormId = getParameter("actionvalue");
	...
	public String printSubTable(String fieldId, String tableSize) {
		...
		jShd.put("width", tableSize + "%");
		jShd.put("height", "200px");
		jShd.put("inserting", true);
		jShd.put("editing", true);
		jShd.put("filtering", false);
		jShd.put("sorting", false);
		jShd.put("paging", false);
		jShd.put("data", "#db_table#");
		...
		while(rs.moveNext()) {
			...
			jsColEl.put("name", fld_name);
			jsColEl.put("width", Integer.valueOf(fld_size));
			if(fld_type.equals("TEXTFIELD")) {
				jsColEl.put("type", "text");
			} else if(fld_type.equals("TEXTAREA")) {
				jsColEl.put("type", "textarea");
			} else if(fld_type.equals("DATEFIELD")) {
				jsColEl.put("type", "myDateField");
			} else if(fld_type.equals("LIST")) {
				String lookups = rs.getString("sub_field_lookup");
				if(lookups != null) {
					jsColEl.put("type", "select");
					jsColEl.put("items", "#db" + fieldId + ".sel_" + fld_name + "#");
					jsColEl.put("valueField", "Name");
					jsColEl.put("textField", "Name");
					jsColEl.put("align", "left");

					JSONObject jsSelObj = new JSONObject();
					JSONArray jsSelModel = new JSONArray();
					String[] lookup = lookups.split("#");
					for(String lps : lookup) {
						jsSelObj.put("Name", lps);
						jsSelModel.put(jsSelObj);
					}
					jsTables.put("db" + fieldId + ".sel_" + fld_name, jsSelModel.toString());
				}
				...
				if(lookups != null) {
					...
					while(lprs.moveNext()) {
						if(cols == 1){
							jsSelObj.put("Id", lprs.readField(1));
							jsSelObj.put("Name", lprs.readField(1));
							jsSelModel.put(jsSelObj);
						} else {
							jsSelObj.put("Id", lprs.readField(1));
							jsSelObj.put("Name", lprs.readField(2));
							jsSelModel.put(jsSelObj);
						}
					}
					lprs.close();
		...
		JSONObject jsColEl = new JSONObject();
		jsColEl.put("width", 75);
		jsColEl.put("type", "control");
		jsColModel.put(jsColEl);
		jShd.put("fields", jsColModel);
		
		String tableDef = jShd.toString().replaceAll("\"#db_table#\"", "db" + fieldId + ".table");	
