...
import org.baraza.server.sms.BSMSModem;
...
public class BServerTimed extends Thread {
	...
	public BServerTimed(String configDir, BElement el, BLogHandle logHandle) {
		...
		if(type.equals("SMSMODEM")) smsModem = new BSMSModem(db, root, logHandle);
		...
		if(type.equals("TOMCAT")) {
			tomcat = new BTomcat(db, root, logHandle, configDir + el.getAttribute("path"));
			tomcat.start();
		}
	...
	public void sendMessage(String inputLine) {
		if(smsModem != null) smsModem.sendMessage(inputLine);
	}

	public String sendMessage(String number, String message, String accesscode) {
		String st = null;
		if(smsModem != null) st = smsModem.sendMessage(number, message, accesscode);
	...
	public void close() {
		...
		if(smsModem != null) smsModem.close();
