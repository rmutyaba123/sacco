...
public class BCrossSet {
	...
	LinkedHashMap<String, Integer> rows;
	LinkedHashMap<Integer, Object> setTable;
	
	public BCrossSet(Vector<Vector<Object>> dataTable) {
		columns = new LinkedHashMap<String, Integer>();
		rows = new LinkedHashMap<String, Integer>();
		setTable = new LinkedHashMap<Integer, Object>();
	...
	public List<String> getTitles() {
		...
		for(String colKey : columns.keySet()) myTitles.add(colKey);
		return myTitles;
	}
	
	public String getHtmlTitles() {
		StringBuffer myhtml = new StringBuffer();
		for(String colKey : columns.keySet()) myhtml.append("<th>" + colKey + "</th>");
	...
	public List<String> getRowList(Object key) {
		...
		if(row == null) {
			...
			for(String colKey : columns.keySet()) { 
				Integer column = columns.get(colKey);
				if(setTable.get((row * 64) + column) == null) rowValues.add("");
				else rowValues.add(setTable.get((row * 64) + column).toString());
			}
		}
