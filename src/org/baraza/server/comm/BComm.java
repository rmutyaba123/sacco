...
public class BComm {
	...
	public void process(BElement el) {
		...
		String smsOrg = el.getAttribute("sms_org");
		String numbers = el.getAttribute("numbers");
		...
		while(rs.moveNext()) {
			...
			String smsOrgId = orgID;
			if(smsOrg != null) smsOrgId = rs.readField(smsOrg);
			if(smsOrgId == null) smsOrgId = orgID;
			
			String smsNumbers = "null";
			if(numbers != null) smsNumbers = rs.readField(numbers);
			if(smsNumbers != null) smsNumbers = "'" + smsNumbers + "'";
			
System.out.println("BASE 3040 : " + sNl + " : " + nf.getError());
			
			// Insert the SMS
			if((sNl == 12) && (nf.getError() == 0) && BNumberFormat.isNumeric(smsNumber)) {
				mysql = "INSERT INTO sms (message_ready, folder_id, org_id, link_id, sms_number, sms_numbers, message) VALUES (";
				mysql += "true, 0, " + smsOrgId + ", ";
				mysql += rs.readField(1) + ", '" + smsNumber + "', " + smsNumbers + ", '" + msg + "');";
...
System.out.println("BASE 3070 : " + mysql);
				...
				mysql = "INSERT INTO sms (message_ready, folder_id, org_id, link_id, sms_number, sms_numbers, message) VALUES (";
				mysql += "true, 0, " + smsOrgId + ", ";
				mysql += rs.readField(1) + ", null, " + smsNumbers + ", '" + msg + "');";
				db.executeQuery(mysql);				
System.out.println("BASE 3050 : " + mysql);
