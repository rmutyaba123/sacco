...
public class BImportModel extends AbstractTableModel {
	...
	public void getExcelData(JPanel panel, String worksheet) { // Get all rows.
		...
		int returnVal = fc.showOpenDialog(panel);
       	if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			Workbook wb = null;
			try {
				if(fileName.indexOf(".xlsx")>1) wb = new XSSFWorkbook(new File(fileName));
				else if(fileName.indexOf(".xls")>1) wb = new HSSFWorkbook(new FileInputStream(fileName));
			} catch (IOException ex) {
				log.severe("an I/O error occurred, or the InputStream did not provide a compatible POIFS data structure : " + ex);
			} catch (InvalidFormatException ex) {
				log.severe("File not of compatible POIFS data structure : " + ex);
			}
	...
	public String getstrvalue(Row row, int column) {
		String mystr = "";
