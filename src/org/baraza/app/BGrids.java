...
class BGrids extends JPanel implements ActionListener, MouseListener {
	...
	public BGrids(BLogHandle logHandle, BDB db, BElement view, String reportDir, boolean nullFilter) {
		...
		editable = view.getAttribute("editable", "false");
	...
	public void importData() {
		...
		if(importProcess) {
			...
			if(impEl != null) {
				...
				tableModel.importData(impQuery.getData(), inParams);
				tableModel.requery();
				impQuery.close();
				impDB.close();
				...
				if(importType.equals("excel")) {
					...
					tableModel.importData(impcsv.getData(), inParams);
					tableModel.requery();
					impcsv.close();
					...
					tableModel.importData(imprec.getData(), inParams);
					tableModel.requery();
					imprec.close();
				}
			}
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if("New".equals(aKey)) {
			...
			for(int selRow : selRows) {
				String selKey = getKey(selRow);
				System.out.println(selRow + " " + selKey);
				actions.execproc(actionList.getSelectedIndex(), selKey, linkValue);
			}
			tableModel.requery();
