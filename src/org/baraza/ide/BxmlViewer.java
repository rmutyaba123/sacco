...
public class BxmlViewer extends JTabbedPane implements ActionListener, TreeSelectionListener {
	...
	public BxmlViewer(BLogHandle logHandle, BDB db, String myFile, String projectName) {
		...
		String[] btArray = {"Save", "Reload", "Find", "Add Attribute", "Delete Attribute", "Web Migration", "App Migration", "Web Cleanup", "Preview", "Language",};
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if("Language".equals(aKey)) {
			tagsMap = new TreeMap<String, String>();
			getLanguageTags(root);
			getLanguageMenu(root.getElement(0));
			for(String tagKey : tagsMap.keySet()) {
				System.out.println(tagKey + "," + tagsMap.get(tagKey));
			}
	...
	public void getLanguageTags(BElement mel) {
		for(BElement smel: mel.getElements()) {
			if(smel.isLeaf()) {
				if(smel.getAttribute("title") != null) {
					tagsMap.put(smel.getValue(), smel.getAttribute("title"));
				}
			} else {
				getLanguageTags(smel);
			}
		}
	}
	
	public void getLanguageMenu(BElement mel) {
		for(BElement smel: mel.getElements()) {
			if(smel.isLeaf()) {
				if(smel.getAttribute("name") != null) {
					tagsMap.put(smel.getAttribute("lang", "menu_" + smel.getValue()), smel.getAttribute("name"));
				}
			} else {
				if(smel.getAttribute("name") != null) {
					tagsMap.put(smel.getAttribute("lang", ""), smel.getAttribute("name"));
				}
				getLanguageMenu(smel);
			}
		}
	}
