...
CREATE TABLE interviews (
	...
	interview_venue			varchar(120) not null,
...
CREATE VIEW vw_interviews AS
	SELECT vw_applications.department_id, vw_applications.department_name, vw_applications.department_description, vw_applications.department_duties,
		...
		vw_applications.entity_id, vw_applications.entity_name, vw_applications.primary_email,
		...
		entitys.entity_id as interviewer_id, entitys.entity_name  as interviewer_name,
		interview_types.interview_type_id, interview_types.interview_type_name,
		interviews.org_id, interviews.interview_id,
		interviews.interview_date, interviews.interview_time, interviews.interview_venue,
		interviews.done, interviews.interview_rating, interviews.details	
...
CREATE OR REPLACE FUNCTION aft_etf_employees() RETURNS trigger AS $$
DECLARE
BEGIN
	IF((NEW.table_code = 101) AND (NEW.is_active = true))THEN
		INSERT INTO e_fields (et_field_id, org_id, table_code, table_id)
		SELECT NEW.et_field_id, NEW.org_id, NEW.table_code, entity_id
		FROM employees
...
CREATE TRIGGER aft_etf_employees AFTER INSERT ON et_fields
	FOR EACH ROW EXECUTE PROCEDURE aft_etf_employees();
...
CREATE OR REPLACE FUNCTION add_interview(varchar(12), varchar(12), varchar(12)) RETURNS varchar(120) AS $$
DECLARE
	v_application_id		integer;
	v_org_id				integer;
	v_interview_type_id		integer;
	v_interview_id			integer;
	v_next_date				date;
	v_next_venue			varchar(120);
	msg		 				varchar(120);
BEGIN

	SELECT application_id, org_id INTO v_application_id, v_org_id
	FROM applications
	WHERE (application_id = $1::int);
	
	SELECT interview_type_id, next_date, next_venue
		INTO v_interview_type_id, v_next_date, v_next_venue
	FROM interview_types WHERE (org_id = v_org_id) AND (is_active = true);
	
	SELECT interview_id INTO v_interview_id
	FROM interviews WHERE (application_id = v_application_id) AND (done = false);

	IF(v_interview_id is null)THEN
		INSERT INTO interviews (application_id, interview_type_id, entity_id, org_id, interview_date,
			interview_time, interview_venue, done)
		VALUES (v_application_id, v_interview_type_id, $2::int, v_org_id, v_next_date,
			'08:30'::time, v_next_venue, false);
		
		msg := 'Added interview';
	ELSE
		msg := 'Interview is already added';
	END IF;
...
CREATE OR REPLACE FUNCTION email_interview(varchar(12), varchar(12), varchar(12)) RETURNS varchar(120) AS $$
DECLARE
	v_interview_id			integer;
	v_org_id				integer;
	v_sys_email_id			integer;
...
BEGIN
	...
	SELECT interview_id, org_id INTO v_interview_id, v_org_id
	FROM interviews WHERE (interview_id = $1::int);

	SELECT sys_email_id INTO v_sys_email_id
	FROM sys_emails 
	WHERE (use_type = 14) AND (org_id = v_org_id);

	INSERT INTO sys_emailed (sys_email_id, org_id, table_id, table_name, email_type)
	VALUES (v_sys_email_id, v_org_id, v_interview_id, 'interviews', v_sys_email_id);

	msg := 'Interview email added';
...
BEGIN
	...
	WHERE (et_fields.org_id = NEW.org_id) AND (et_fields.table_link = NEW.interview_type_id) 
		AND (et_fields.table_code = 121) AND (et_fields.is_active = true);
...
BEGIN
	...
	SELECT sys_email_id INTO v_sys_email_id
	FROM sys_emails 
	WHERE (use_type = 9) AND (org_id = v_org_id);
	
	INSERT INTO sys_emailed (sys_email_id, org_id, email_type, narrative)
	VALUES (v_sys_email_id, v_org_id, 9, 'Its birthday for ' || v_entity_name);
