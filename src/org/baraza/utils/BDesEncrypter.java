...
public class BDesEncrypter {
	...
	public static void main(String args[]) {
		...
		if (args.length > 2) {
			...
			String encryptionKey = args[1].trim();
			String fileName = args[2].trim();
			
			// Create encrypter/decrypter class
			BDesEncrypter encrypter = new BDesEncrypter(encryptionKey);

			if(cmd.equals("encrypt")) {
				System.out.println("Encrypting file : "  + fileName + ".xml");
				
				// Encrypt
				encrypter.encrypt(fileName + ".xml", fileName + ".cph");
			} else if(cmd.equals("decrypt")) {
				System.out.println("Decrypting file : "  + fileName + ".cph");
				
				// Decrypt
				encrypter.decrypt(fileName + ".cph",  fileName + ".new.xml");
			}
		} else {
			System.out.println("run as");
			System.out.println("java -cp baraza.jar org.baraza.utils.BDesEncrypter encrypt encryptionKey fileName");
			System.out.println("java -cp baraza.jar org.baraza.utils.BDesEncrypter decrypt encryptionKey fileName");
		}
	}
