...
public class BWebDashboard {
	...
	public String getLink(BElement el) {
		StringBuffer body = new StringBuffer();
		
		String icon = el.getAttribute("icon", "fa fa-arrow-right");

		body.append("<a class='link_btn' href='" + el.getAttribute("url", "") + "?view=" + el.getAttribute("view", "1:0"));
		if(el.getAttribute("data") != null) body.append("&data=" + el.getAttribute("data"));
		body.append("'");
		if(el.getAttribute("blankpage", "false").equals("true")) body.append(" target='_blank'");
		body.append("><i class='" + icon + "'></i> " + el.getAttribute("title") + "</a>");
		return body.toString();
	}
