...
public class BDesk extends JInternalFrame implements MouseListener {
	...
	public BDesk(BLogHandle logHandle, BDB db, BElement desk, String reportDir) {
		...
		accordions = new ArrayList<BAccordion>();
		...
		for(BElement el : desk.getElements()) {
			...
			} else if(el.getName().equals("ACCORDION")) {
				accordions.add(new BAccordion(logHandle, db, el, reportDir));
				tabbedPane.addTab(el.getAttribute("name"), accordions.get(accordions.size() -1));

				tabs.add(new BTabs(2, accordions.size()-1));
			}
		}
		add(tabbedPane);
