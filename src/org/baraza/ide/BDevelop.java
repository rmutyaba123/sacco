...
public class BDevelop extends JInternalFrame implements ActionListener {
	...
	BQBuilder qbuilder;
	BxmlViewer viewer;
	BDataImport dataImport;
	BDBUpgrade DBUpgrade = null;
	...
	public void makeDeskItems() {
		...
		String projectDir = configDir + desk.getAttribute("path") + ps;
		String dbFilesDir = projectDir + "database" + ps;
		String configFile = projectDir + "configs" + ps + desk.getAttribute("xmlfile");
		String reportDir = projectDir + "reports" + ps;
		...
		dataImport = new BDataImport(logHandle, db, projectDir);
		DBUpgrade = new BDBUpgrade(logHandle, db, desk, dbFilesDir);

		tabbedPane.add("Query Builder", queryTabs);
		tabbedPane.add("XML Viewer", viewer);
		tabbedPane.add("Data Import", dataImport);
		tabbedPane.add("DB Upgrade", DBUpgrade);
		
		queryTabs.add("Query", qbuilder);
		if(!dbFilesDir.startsWith("http")) {
			edit = new BEdit(dbFilesDir, db, logHandle);
	...
	public void close() {
		if(DBUpgrade != null) DBUpgrade.close();
		if(db != null) db.close();
		dispose();
	}
