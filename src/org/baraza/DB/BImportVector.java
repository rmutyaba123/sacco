...
public class BImportVector {
	...
	public BImportVector(BElement view) {
		...
		copyTopValue = new ArrayList<Boolean>();
		...
		for(BElement el : view.getElements()) {
			if(el.getAttribute("title") != null) {
				...
				if(el.getAttribute("copy.topvalue", "false").equals("true")) copyTopValue.add(true);
				else copyTopValue.add(false);
			}
       	}
	}
					...
					if(mytokens.length>0) {
						...
						for (int j = 0; j < getColumnCount(); j++) {
							if(j < mytokens.length) {
								String mytoken = mytokens[j];
								if(mytoken.startsWith("'")) mytoken = mytoken.substring(1);
								myvec.add(getstrvalue(mytoken));
							} else {
								myvec.add("");
							}
						}
	...
	public void getExcelData(InputStream input, String fileName, String worksheet, Integer firstRow) { // Get all rows.
		...
		String myline = "";
		Map<Integer, String> cellVals = new HashMap<Integer, String>();
		for(i = firstRow; i <= sheet.getLastRowNum(); i++) {
			...
			if(row != null)  {
				...
				for (int j=0; j<getColumnCount(); j++) {
					...
					if(copyTopValue.get(j)) {	// Login to keep cell value copy
						if(cellVal.equals("")) {
							if(cellVals.containsKey(j)) {
								cellVal = cellVals.get(j);
								if(j == 0) myline = cellVal;
							}
						} else {
							cellVals.put(j, cellVal);
						}
					}
